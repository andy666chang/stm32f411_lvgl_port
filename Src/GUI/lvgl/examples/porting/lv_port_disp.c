...
void lv_port_disp_init(void)
{
    ...
    static lv_color_t draw_buf_1[LV_HOR_RES_MAX * LV_VER_RES_MAX];                          /*A buffer for LV_VER_RES_MAX rows*/
...
//    }
		...
		#ifdef _PART_SCREEN
		for( i = (area->y1>>3) ; i <= (area->y2>>3) ; i++ ) // LV_VER_RES_MAX line (page)
		{
				SSD1306_Set_XY( area->x1 , i );
				for( k = (area->y1&0x07) ; k <= (area->y2&0x07) ; k++) // LV_VER_RES_MAX line (in page)
				{
