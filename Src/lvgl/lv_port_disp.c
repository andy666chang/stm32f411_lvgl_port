...
#ifdef _PART_SCREEN
uint8_t frame_pixel[LV_VER_RES_MAX >> 3][LV_HOR_RES_MAX] = {0} ;
#endif
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    if ( area->y1 == 0 && area->y2 == LV_VER_RES_MAX - 1 && area->x1 == 0 && area->x2 == LV_HOR_RES_MAX - 1 )
    {
...
#ifdef _PART_SCREEN
		// set write range
		SSD1306_Set_Writed_Area( area->x1 , area->x2 , area->y1 /8 , area->y2/8 );

		
    for ( i = (area->y1 /8 ) ; i <= (area->y2 /8 ) ; i++ ) // LV_VER_RES_MAX line (page)
    {
//        SSD1306_Set_XY( area->x1 , i );
			uint8_t y_start = ( area->y1+1 > i*8 )?  area->y1 - i*8 : 0 ;
			uint8_t y_end   = ( area->y2+1 > (i+1)*8 )?  7 : area->y2-(i*8) ;
        for ( k = y_start  ; k <= y_end ; k++) // LV_VER_RES_MAX line (in page)
        {
            for ( j = area->x1 ; j <= area->x2 ; j++ )  // LV_HOR_RES_MAX
            {
                (lv_color_to1(*color_p)) ? SET_BIT( frame_pixel[i][j] , 1 << (k) ) : CLEAR_BIT( frame_pixel[i][j] , 1 << (k) );
                color_p++;
            }
        }
        SSD1306_Write( SSD1306_SLAVE_ADDRESS , NORMAL_MODE_ADDRESS , &frame_pixel[i][area->x1] , area->x2 - area->x1 + 1 );
    }
#endif
